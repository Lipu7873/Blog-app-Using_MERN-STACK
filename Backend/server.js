...
import serviceAccoufntKey from "./react-blog-website-yt-firebase-adminsdk-v0305-7302e89e50.json" assert{type:"json"}
...
admin.initializeApp({
    credential: admin.credential.cert(serviceAccoufntKey)
})
...
mongoose
  ...
  // setting up s3 bucket //AWS database connention
  const s3 = new aws.S3({
    region: "ap-south-1",
...
app.post("/signup", (req,res)=>{
    ...
    if (!passwordRegex.test(password)){
        return res.status(403).json({"error":"Password should be 6 to 8 characters long with a numeric, 1 lowercase, and 1 uppercase letters"})
    }
...
app.post("/get-blog-comments", (req, res) => {
  ...
  Comment.find({ blog_id, isReply: false }).populate(
    "commented_by",
